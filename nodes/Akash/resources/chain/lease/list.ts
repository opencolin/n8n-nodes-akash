...
/**
 * Chain → List Leases — `GET /akash/market/v1beta5/leases/list`.
 *
 * KEYLESS on-chain read (Cosmos LCD, no `x-api-key`, no spend). Lists leases from
 ...
 * (`active`|`insufficient_funds`|`closed`). String filters are omitted when blank;
 * the `gseq`/`oseq` sequence numbers are omitted when 0 (Akash sequences start at 1,
 * so 0 means "unset"). `paginateChain` owns `returnAll`/`limit` + `next_key`.
 */
export async function executeChainListLeases(
	...
	return paginateChain.call(this, chainPaths.leasesList(), 'leases', filters, itemIndex);
}
